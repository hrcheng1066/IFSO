...
def feed_forward_once(model):
    ...
    inputs = torch.zeros(1, 3, 32, 32).cuda(cuda_id)  # cheng
    neck_out = model.module.backbone(inputs)
    temp = neck_out[-1].sum()
    return temp
...
class FisherPruningHook(Hook):
    ...
    def before_train_iter(self, runner):
        ...
        for module, name in self.nonpass_names.items():  # added for fc
            flops_backup[module] = self.flops[module]
            acts_backup[module] = self.acts[module]
        ...
        for i, data_batch in enumerate(runner.data_loader):
            ...
            if count > 1:  # x/2
                ...
                for module, name in self.nonpass_names.items():
                    ...
                    index += 1
                break
        # recover flops and acts before run_iter in epoch_based_runner.py
        for module, name in self.nonpass_names.items():  # added for fc
            self.flops[module] = flops_backup[module]
            self.acts[module] = acts_backup[module]
    ...
    def print_model(self, runner, print_flops_acts=True, print_channel=True):
        ...
        if print_flops_acts:
            ...
            # when the two thrs are achieved, then exit this func in advance.
    ...
    def single_prune(self, info, exclude=None):
        ...
        for module, name in self.nonpass_names.items():  # added for fc
            ...
            # for debug
            if 'features.0.conv' in name and fisher.sum() != 0:
                print("ok")
