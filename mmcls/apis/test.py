...
def single_gpu_test_inference_time(model, data_loader):
    model.eval()
    total_mean = 0
    for i, data in enumerate(data_loader):
        # the following code is used to measure inference time
        ...
        for _ in range(10):
            result = model(return_loss=False, **data)
            #result = model(return_loss=False, **input)
